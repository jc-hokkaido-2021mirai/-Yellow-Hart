...
class Former {
    ...
    /**
     * [SET] トリガー作成
     * スイッチの役割をするトリガーを作成します
     * 
     * @param string $id トリガーに対して付けるIDを指定します
     * @param string $name トリガー名を指定します
     * @param string $caption このトリガーに対する説明を指定します
     * @param bool $openflag 対象トリガーの初期状態ON/OFFを指定します（ON: true, OFF: false）
     */
    function Trigger($id, $name, $caption, $openflag) {
	$opentext = ($openflag) ? 'checked' : '';
	$this->FormStart($name, 'toggle-on');
	array_push($this->data, '<div class="switchArea"><input type="checkbox" id="' . $id . '" ' . $opentext . '><label for="' . $id . '"><span></span></label><div id="swImg"></div></div>');
	$this->FormEnd();
    }
